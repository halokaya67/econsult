...
That opens `specs/001-econsult-flow/index.html` in your browser: a dashboard of the loop in
...
| ---------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Research with source verification              | Library behaviour recalled confidently and wrongly                | `flow/research.md` — a verdict and an unverified list per topic; it caught that the classic form-data file part it first recommended is rejected by Expo's `fetch` on SDK 57            |
| Spec and plan checker rounds                   | Drift from the brief, and plans that cannot be built              | `flow/checker/spec-r1.md`–`spec-r5.md` and `flow/checker/plan-r1.md`–`plan-r3.md` — one round caught screen tests placed under `src/app`, where expo-router loads every file as a route |
| One commit per task, tests first, file-fenced  | Scope creep, and code that ships without a test                   | The execution log at the end of `flow/plan.md`, and the 100 % coverage thresholds in `package.json`                                                                                     |
...
| The final three-round implementation review    | Regressions across the whole diff rather than inside one step     | `flow/checker/final-r1.md`, `flow/checker/final-r2.md`, `flow/checker/final.md`                                                                                                         |
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| `index.html`                 | The entry page: every page below links from it, and it needs no network                                                                                                                             | Start here — open it in a browser from the clone         |
...
| `artifacts/checker/`         | The review rounds, rendered from the markdown in `flow/checker/`                                                                                                                                    | To read a verdict and what it blocked                    |
| `flow/research.md`           | The platform facts checked before planning, with the adversarial re-checks that overturned some of them                                                                                             | Before questioning a platform choice                     |
| `flow/how-it-works.md`       | The repository as it stood before this feature                                                                                                                                                      | To see what the plan had to build on                     |
| `flow/spec.md`               | The agreed behaviour, settled after the five spec rounds                                                                                                                                            | Before reading the code                                  |
| `flow/plan.md`               | Twenty-two test-first steps with their file fences and verify commands; the execution log at the end names each step's commit, its amendment and its visual outcome, and a decision index closes it | To map a commit back to the step that produced it        |
| `flow/checker/`              | The rounds themselves: five on the spec, three on the plan, three on the finished change                                                                                                            | When a rendered report raises something you want in full |
