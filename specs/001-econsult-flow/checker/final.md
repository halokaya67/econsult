...
| W6 — SC3 Accessibility Inspector evidence | DEFERRED to finish:ui-verify (still open) | `spec.md:203` names the audit; `README.md:113` tells reviewers to run it; grep → 0 reports mention it. The UI stage must drive: Xcode > Open Developer Tool > Accessibility Inspector > Audit on Home, step 1 (loaded and empty), step 2, step 3 (pristine and with the error shown), Sent (partial variant), Developer settings; record the result per screen — or amend SC3 and DECISIONS.md to name the agent-device a11y dump as the audit |
...
- **Suggested fix:** in finish:ui-verify, run the Xcode Accessibility Inspector audit on Home, step 1 (loaded and empty), step 2, step 3 (pristine and with the error shown), Sent (partial variant) and Developer settings; record the per-screen result. If it cannot be driven, amend SC3 to name the agent-device a11y dump as the audit and say so in DECISIONS.md.
- **Needs user:** no
