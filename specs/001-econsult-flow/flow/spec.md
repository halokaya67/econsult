...
> **Revised after implementation.** The text below describes the app as it was built. Six things were settled differently from the version agreed at the spec gate:
>
> - Source layout: the flow splits into `api`, `state`, `hooks`, `components` and `utils` under `src/features/econsult`, and the shared pieces sit in `src/api`, `src/providers`, `src/hooks` and `src/lib` rather than in `src/lib` alone.
> - The developer settings screen is the only knob: no `.env`, no `EXPO_PUBLIC_*` seeds.
> - A validation error is spoken once, by the focus move to the field whose accessible name carries it; no error surface is a live region.
> - The screen scaffold pads for the keyboard itself instead of using `automaticallyAdjustKeyboardInsets`, which applies other apps' keyboard frames too.
...
The deliverable is a repository that runs from a clean clone with `npx expo start` in Expo Go on the iOS simulator, plus DECISIONS.md and AI-USAGE.md. The screen recording is made by hand after the flow finishes. DECISIONS.md records where the app was verified: the iOS simulators, an Android 16 emulator with TalkBack's speech captured, a real Samsung tablet and a real iPad.
...
- **Practice switching and failure injection.** A development-only settings screen is the only knob. It switches between the three fixture practices at runtime, sets latency, forces any of the four requests to fail (network, server or timeout) and forces offline. Its starting state is `DEFAULT_DEV_SETTINGS` in `src/providers/DevSettingsProvider.tsx`; no environment variable feeds the fake. Applying the settings clears the query cache and returns home, so the next entry into the flow fetches fresh.
...
- **Keyboard and safe areas.** Core React Native only: one `ScrollView` per step with the primary button as the last child inside the scroll content, `keyboardShouldPersistTaps="handled"`, the scaffold's own keyboard listener padding the scroll content, and bottom insets from `useSafeAreaInsets`. React Native's `automaticallyAdjustKeyboardInsets` was tried and dropped: iOS reports the out-of-process photo picker's keyboard frame too, which blanked the message step on a real iPad, so the scaffold listens for the keyboard itself and ignores foreign and empty frames. expo-router already mounts the safe-area provider and the header consumes the top inset. react-native-keyboard-controller was considered and rejected: the Expo docs contradict themselves on its Expo Go status, and the baseline needs no native module.
...
| Area | Change |
|---|---|
| `src/app/_layout.tsx` | Mounts `AppProviders` (developer settings, query client, network, services) around the root stack; sets stack defaults (minimal back button), titles `index` "Your practice", declares `dev-settings` a modal, and sets `headerShown: false` for the `econsult` group so the nested stack owns the header. |
...
| `src/app/+not-found.tsx` (new) | The unmatched-route screen: a heading, one line of explanation and a button home. A button rather than a `Link`, which Android renders as plain text a screen reader skips. |
| `src/api/` (new) | Contracts, the transport interface, the fake transport, three services, fixtures. |
| `src/features/econsult/` (new) | The flow in five kinds: `state/` (draft reducer and provider), `api/` (query options and the send path), `hooks/` (what the screens call), `components/` (recipient card, photo picker, step header), `utils/` (recipients, steps, validation, error copy, photo processing). |
| `src/features/devSettings/` (new) | `utils/options.ts`: the option tables the developer settings screen renders. |
| `src/components/` (new) | Screen scaffold, buttons, text field, choice group, offline banner, and the loading, empty and error states as three files. |
| `src/providers/` (new) | The one provider stack (`AppProviders`) and the three providers it mounts: developer settings, network link state, and the services composition root. |
| `src/hooks/` (new) | `useSession`, the faked patient session. |
| `src/lib/` (new) | Accessibility announcement and focus helpers, field labels, the shared retry label, the in-flight guard, ids, `devWarn`. |
| `src/test/` (new) | The test-only provider stack, the flow layout that carries a preset draft, the handled-promise helper, and ambient types for expo-router's runtime matchers. |
...
| `package.json` | Adds expo-image-picker, expo-image-manipulator, expo-network, expo-crypto, @tanstack/react-query and zod 4; adds the test, lint, format and type-check tooling with hand-pinned versions where Expo's install command would fetch incompatible ones (@testing-library/react-native 13.3.3 exact, jest on the 29 line, @react-native/jest-preset ^0.86.3); removes packages the app never imports (expo-font, expo-status-bar, react-native-web, react-dom, react-native-gesture-handler, react-native-reanimated, react-native-worklets) and the `web` script. expo-device stays: it is the source of `isDevice`. |
...
4. The developer settings screen, the one way to switch practice, latency and faults.
5. Dependency and asset cleanup, then the three documents.

If time runs out, item 4 is cut first, then the cleanup inside item 5. The three documents are never cut, and DECISIONS.md names whatever was.
...
- Photo still preparing: Send is disabled with reason; the source actions are hidden; Remove discards the pending result and shows the source actions again, so a pick started after Remove supersedes the pending one. The draft holds `photo` as `{ status: 'preparing', pickId }` or `{ status: 'ready', pickId, uri, width, height }`, and a processing result is ignored unless its `pickId` matches the draft's. The preview renders at the block's fixed height, at most 360 points wide and aligned to the start, with `contentFit: 'cover'`, and never sizes itself from `width` and `height`, so a `ready` photo carrying a 0 dimension still shows.
...
- Default simulated latency: config 2000 ms, care team 1000 ms, create 1000 ms, upload 1000 ms, each adjustable from the developer settings screen.
...
- The fake transport can be told, per endpoint, to fail with `network`, `server` or `timeout`, and to add latency, from the development settings screen. Query keys include the practice id, so switching practice never serves another practice's cached data.
...
- **Screen readers.** Every control has an accessible name; errors are tied to their field by folding the error into the accessible name of the field's wrapping accessible view (iOS caches a text's own name until its words change) and moving screen-reader focus there, so the error is spoken once, by the focus move. No error surface is a live region. Recipient cards are grouped as radio buttons in a radio group with `checked` state. Form fields are never grouped, so each stays individually focusable. Headings use the header role. Screen-reader focus moves to the first invalid field on validation failure and to the heading on the confirmation screen.
- **Large text.** `allowFontScaling` stays on everywhere; `maxFontSizeMultiplier` is used only on the step counter chip and on buttons placed inside the native navigation bar, whose height iOS fixes (the same cap iOS applies to its own bar buttons); their accessible names stay complete. Buttons grow with their text. The native header keeps a short title; the real heading sits in the scrollable body where it can wrap.
...
- **iOS versus Android.** `gestureEnabled: false` only works on iOS, so the confirmation screen also uses `usePreventRemove` for Android's back gesture. Live regions are Android only, which is one reason the app does not use them for errors; announcements are used on both. The iOS simulator has no camera; the Android emulator has a virtual scene camera that can be fed an image. `keyboardDismissMode` is `interactive` on iOS and `on-drag` on Android.
...
| Photo downscaling | expo-image-manipulator (new dependency, included in Expo Go) | one small `processPhoto` function in `src/features/econsult/utils` |
...
| Accessibility | `AccessibilityInfo.announceForAccessibility`, `AccessibilityInfo.sendAccessibilityEvent(ref, 'focus')`, `accessibilityRole`, `accessibilityState`, `useWindowDimensions().fontScale`, `Pressable` | announcement and focus helpers in `src/lib`, beside the small label helpers (`fieldLabel`, `retryLabel`) |
