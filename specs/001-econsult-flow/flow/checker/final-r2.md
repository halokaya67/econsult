...
> - W6, deferred here to the UI-verification phase, is closed: the Accessibility Inspector audit ran on the phone lane.
> - The one function over 50 lines is gone; there are none.
...
| W6 — SC3 Accessibility Inspector evidence | DEFERRED to finish:ui-verify | Must be driven: Xcode Accessibility Inspector audit on Home, step 1 (loaded and empty), step 2, step 3 (pristine and with the error shown), Sent (partial variant), Developer settings; record the result per screen, or amend SC3 + DECISIONS.md if the accessibility-tree dump is to stand in for it |
