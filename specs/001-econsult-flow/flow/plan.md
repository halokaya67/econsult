...
- 2026-09-12, during T018 — the simulator preview of the message step found that an empty Send at the largest text size scrolled the input to the top of the viewport and left the field's label far above the fold: `TextField` forwards its `ref` to the inner `TextInput`, so the scroll-to-field hook measured the input rather than the field group (text questions on the questions screen have the same weakness; the choice group did not, because its ref lands on the label). Lead decision: `TextField` gains an optional `containerRef` on its wrapping `View`, the scroll anchor, while `ref` stays on the input for screen-reader focus; the message and questions screens scroll to the anchor and focus the input. The same preview showed the multiline input's fixed minimum height leaving roughly one and a half lines visible at the largest size, so the minimum height now scales with the system font scale. T018's fence gains `src/components/TextField.tsx`, `src/components/TextField.test.tsx`, `src/app/econsult/questions.tsx` and `src/__tests__/app/econsult/questions.test.tsx`. The offline-banner state of this step's preview is taken in T020's preview, where the developer-settings toggle that forces offline exists.
