...
- Copy is English. Light appearance only. System font. Every interactive element has `minHeight` and `minWidth` of at least 48 (`MIN_TOUCH` in `src/theme/tokens.ts`); `allowFontScaling` is never set to false; `maxFontSizeMultiplier` is used only on the step counter chip.
