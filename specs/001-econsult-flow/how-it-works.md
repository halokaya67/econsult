...
- **Networking primitives**: `fetch` is whatwg-fetch 3.6.20 and honours `AbortSignal`, rejecting with an `AbortError`; `AbortController` comes from the abort-controller 3.0.0 polyfill and has no `AbortSignal.timeout`, so a timeout is a `setTimeout` that calls `abort()`; `FormData` accepts `{ uri, name, type }` parts so a file can be uploaded straight from its URI.
- **SDK modules bundled in Expo Go but not yet installed**: `expo-image-picker ~57.0.17`, `expo-network ~57.0.2` and `expo-file-system`, all pinned in `expo/bundledNativeModules.json`, so `npx expo install` adds them without a development build.
